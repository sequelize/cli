        ...
        gulp
          ...
          .pipe(helpers.expect(function (stdout) {
            if (_.includes(flag, 'coffee')) {
              expect(stdout).to.contain('up: (queryInterface) ->');
              expect(stdout).to.contain('down: (queryInterface) ->');
            } else {
              expect(stdout).to.contain('up: function (queryInterface) {');
              expect(stdout).to.contain('down: function (queryInterface) {');
            }
          }))
          .pipe(helpers.teardown(done));
      });
