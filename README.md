...
The default files generated by the CLI for new migrations and seeders can be overridden. To override these default files, place a custom template named `skeleton.js` within your project's migrations or seeders directories.

Note that a skeleton must export both an `up` and a `down` function, and that both `up` and `down` should return a promise handling any asynchronous calls.

For reference, the default migration skeleton is:
...
For reference, the default seeder skeleton is:
