...
module.exports = {
  'db:migrate': {
    ...
    task: function () {
      getMigrator('migration', function (migrator) {
...
function getMigrator (type, callback) {
  if (helpers.config.configFileExists() || args.url) {
    ...
    var coffeeRegex = /^(?!skeleton)(.*)\.js$|\.coffee$/;
    var jsRegex = /^(?!skeleton)(.*)\.js$/;
    var migrator  = new Umzug({
      ...
      migrations:     {
        ...
        pattern: helpers.config.supportsCoffee() ? coffeeRegex : jsRegex,
        wrap:    function (fun) {
          if (fun.length === 3) {
